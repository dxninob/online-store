<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Computer Language Lines
    |--------------------------------------------------------------------------
    |
    | The following language lines are used by the paginator library to build
    | the computer application messages. You are free to change them to anything
    | you want to customize your views to better match your application.
    |
    */

    'cpu' => 'CPU',
    'ram' => 'RAM',
    'gpu' => 'GPU',
    'storage' => 'Almacenamiento',
    'categories' => 'Categorías',

    'index.title' => 'Computadores - Tienda Online',
    'index.subtitle' => 'Lista de computadores',
    'index.sortPrice' => 'Filtrar por precio',
    'index.sortPrice.removeFilter' => 'Quitar filtro',
    'index.sortPrice.lowToHigh' => 'Menor a mayor',
    'index.sortPrice.highToLow' => 'Mayor a menor',
    'index.sortCategory' => 'Filtrar por categoria',
    'index.filter' => 'Filtrar',

    'show.title' => ' - Tienda Online',
    'show.subtitle' => ' - Information del Computador',
    'show.quantity' => 'Cantidad',
    'show.shareText' => 'Te comparto este computador, creo que te podría interesar: ',
    'show.share' => 'Compartir en Whatsapp',
    'show.addToOrder' => 'Añadir a la orden'
];